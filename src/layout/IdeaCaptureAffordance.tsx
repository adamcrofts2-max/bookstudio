...
export function IdeaCaptureAffordance({ projectId }: IdeaCaptureAffordanceProps) {
  ...
  if (!expanded) {
    return (
      <button
        ...
        className="absolute bottom-4 right-4 z-20 flex size-10 items-center justify-center rounded-full border border-border bg-panel/95 text-text-secondary shadow-[var(--shadow-md)] backdrop-blur transition-colors duration-150 hover:text-[var(--color-accent)]"
      >
  ...
  return (
    <div className="absolute bottom-4 right-4 z-20 flex w-72 flex-col gap-2 rounded-[var(--radius-card)] border border-border bg-panel/95 p-3 shadow-[var(--shadow-md)] backdrop-blur">
