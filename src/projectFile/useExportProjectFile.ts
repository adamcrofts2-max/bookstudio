 ...
 * (manuscript, structural pages, notes, custom theme, image assets) into a
 * single `.bookstudio` archive via `exportProjectFile.ts` and saves it to
 * the user's computer via the shared `saveBlob` helper, same shape as
 * `pdf/useExportPdf.ts`/`epub/useExportEpub.ts`. Unlike those, `canExport`
 * only requires the project to exist — even a brand-new, empty project is
 * worth being able to save and resume later.
 */
